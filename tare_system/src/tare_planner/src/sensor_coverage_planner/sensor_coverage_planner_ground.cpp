...
bool PlannerParameters::ReadParameters(ros::NodeHandle& nh)
{
  ...
  // Legacy LOAM key poses use a cyclic XYZ convention.  DCL/FAST-LIO poses
  // are already in standard ROS XYZ, so the real-robot launch disables this.
  kRotateKeypose = misc_utils_ns::getParam<bool>(nh, "kRotateKeypose", true);
